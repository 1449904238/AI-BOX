...
<template>
  <ul class="general flex flex-col gap-4">
    <li>
      ...
      <a-radio-group v-model="currentLang" type="button" size="large" @change="(value: any) => setLanguage(value)">
        <a-radio value="zh">
          <icon-chinese-fill />
        </a-radio>
        <a-radio value="en">
    ...
    <li>
      ...
      <a-radio-group v-model="themeMode" type="button" size="large" @change="(value: any) => toggleTheme(value)">
        <a-radio value="light">
          <icon-sun />
        </a-radio>
        <a-radio value="dark">
    ...
    <li>
      ...
      <a-tooltip :content="$t('setting.general.proxyMadolTip')" position="rt">
        <icon-info-circle-fill size="22" />
      </a-tooltip>
    ...
    <li v-show="proxy.bypass">
      ...
      <a-input v-model="proxy.url" :placeholder="$t('setting.general.proxyAddress')"></a-input>
    </li>

    <div class="flex justify-end text-sm text-[var(--color-text-3)]" v-show="proxy.bypass">
      {{ $t('setting.general.example') }}：https://api.openai-proxy.com<span class="mark"></span>
    </div>
  </ul>
</template>
...
.general {
  li {
    ...
    >i {
      --uno: cursor-default min-w-50 text-right not-italic text-[var(--color-text-2)];
    }
