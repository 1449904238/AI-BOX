...
const allRenderList = computed(() => {
  renderList.value = filterRoleList.value.length && !isAdd.value
    ? [...filterRoleList.value]
    : [...roleList.value]
  return renderList.value
})

watch(allRenderList, () => handleVisible(!!allRenderList.value.length))
...
const searchRole = () => {
  const arr = allRenderList.value.filter((res: any) => {
    return res.name.includes(keyword.value)
  })
  // console.log(arr)
...
<!-- TODO: 优化代码 -->
<template>
  <a-popover :title="$t('session.role.select')" class="role-popover w-full" trigger="click"
    :popup-visible="popoverVisible" @popup-visible-change="handleVisible">
    <template #title>
      <div class="flex items-center justify-between">
        <div>
          ...
          <a-button type="outline" @click="handleAdd" v-if="!isAdd" style="margin-left: 10px;">
            {{ $t('session.role.add') }}
          </a-button>
        </div>
        <!-- 搜索过滤失效，暂时没找到原因 -->
        <!-- <a-input class="search" v-model="keyword" @input="searchRole" :placeholder="$t('session.role.search')" allow-clear
          auto-focus /> -->
        <icon-double-down style="cursor: pointer;" @click="handleClick" size="24" />
      </div>
    </template>

    <a-tooltip :content="currentRole?.name" position="tl">
      <Avatar class="w-12! cursor-pointer" :value="currentRole?.name" @click="handleClick" />
    </a-tooltip>

    <template #content>
      <div class="renderList">
        <div v-for="item in renderList"
          class="transition-300 flex cursor-pointer items-center gap-4 bg-transparent p-4 text-[var(--color-text-1)] hover:bg-[var(--color-fill-2)] bordered renderListItem"
          :class="{
            'bg-[rgb(var(--blue-6))]!': currentRole?.id === item.id,
            'bg-transparent!': item.isEdit,
            'text-white': currentRole?.id === item.id && !item.isEdit
          }" :key="item.id" @click="handleSelect(item)">
          <div class="flex flex-1 items-center gap-4">
            ...
            <div class="flex flex-1 flex-col gap-2">
              ...
              <template v-else>
                <a-input v-model="item.name" :placeholder="$t('session.role.namePlaceholder')" allow-clear auto-focus />

                <a-textarea v-model="item.description" :placeholder="$t('session.role.descriptionPlaceholder')" auto-size
                  allow-clear />
              </template>
            </div>
          </div>
          ...
          <div v-if="!item.is_default" @click.stop>
            <div v-if="!item.isEdit" class="text-5 flex gap-5">
              ...
              <a-popconfirm type="error" position="tr" :content="$t('session.role.confirmDelete')"
                @ok="deleteRole(item.id!)" :ok-text="$t('common.confirm')" :cancel-text="$t('common.cancel')">
                <icon-delete />
              </a-popconfirm>
            </div>
