...
<template>
  <a-modal :visible="visible" simple :title="$t('setting.about.version.newVersion')" :mask-style="modalMaskStyle"
    :ok-text="isDownload
      ? $t('setting.about.version.updating')
      : $t('setting.about.version.updateNow')
      " :cancel-text="isDownload
    ? $t('setting.about.version.cancelUpdate')
    : $t('setting.about.version.updateLater')
    " :mask-closable="false" :esc-to-close="false" @before-ok="handleBeforeOk" @cancel="handleCancel(false)">
    <div class="flex flex-col gap-4">
      ...
      <span>{{ $t('setting.about.version.detail')
      }}<a href="https://github.com/Synaptrix/ChatGPT-Desktop/releases/latest">Github</a></span>
    </div>
  </a-modal>
</template>
